...
const Blog = ({ data }) => {
  ...
  const categories =
    data && data.categories
      ? ["الكل", ...data.categories.map((cat) => cat.name)]
      : ["الكل"];
      ...
      <div className="container py-5">
        <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
          <p className={styles.resultsCount}>
            عرض{" "}
            <span className="fw-bold text-white">{filteredPosts.length}</span>{" "}
            مقالات
        ...
        >
          {currentPosts.map((post) => (
            ...
            >
              <article
                className={`${styles.articleCard} ${viewMode === "list" ? styles.cardList : ""}`}
              >
