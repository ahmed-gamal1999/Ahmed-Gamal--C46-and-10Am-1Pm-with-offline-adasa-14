...
const NavBar = () => {
  return (
    <nav
      className={`navbar navbar-expand-lg ${navBarCss.navbarCustom} sticky-top`}
      dir="rtl"
    >
      <div className="container-fluid max-w-7xl px-4 position-relative d-flex justify-content-between align-items-center">
        <NavLink className={`navbar-brand ${navBarCss.brandContainer}`} to="/">
          <div className="d-flex flex-column">
            ...
            <span className={`d-none d-sm-block ${navBarCss.brandSubtitle}`}>
        ...
        <div className={`d-none d-lg-flex ${navBarCss.desktopNavCentered}`}>
          <div className={navBarCss.desktopPillsWrapper}>
            <NavLink
              to="/home"
              className={({ isActive }) =>
                `${navBarCss.desktopLink} ${isActive || location.pathname === "/" ? navBarCss.desktopLinkActive : ""}`
              }
            >
              الرئيسية
            </NavLink>
            <NavLink
              to="/blog"
              className={({ isActive }) =>
                `${navBarCss.desktopLink} ${isActive ? navBarCss.desktopLinkActive : ""}`
              }
            >
              المدونة
            </NavLink>
          ...
          <div className={navBarCss.mobileMenuBox}>
            <div className="d-flex flex-column gap-1">
              <NavLink
                to="/home"
                className={({ isActive }) =>
                  `${navBarCss.mobileLink} ${isActive ? navBarCss.mobileLinkActive : ""}`
                }
              >
                الرئيسية
              </NavLink>
              ...
              <NavLink
                to="/start"
                className={`${navBarCss.ctaButtonMobile} mt-2`}
              >
                ابدأ القراءة
              </NavLink>
            </div>
